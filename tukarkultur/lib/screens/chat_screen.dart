...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _addCulturalWelcomeMessage() async {
    ...
    // Get cultural background information from OpenAI
    try {
      ...
      final conversationStarter = await _getConversationStarter();
      
      _addLocalMessage(ChatMessage(
        sender: 'AI Assistant',
        receiver: 'user_${DateTime.now().millisecondsSinceEpoch}',
        text: '💡 Here are some conversation starters:\n\n$conversationStarter',
        timestamp: DateTime.now(),
        isMe: false,
      ));
  ...
  Future<String> _getCulturalInsight() async {
    // Determine likely cultural background from name (simplified approach)
...
''';
    ...
    try {
      ...
      return response?.response ?? _getFallbackCulturalInsight();
    } catch (e) {
      print('OpenAI cultural insight failed: $e');
